...
const blSendToMAWDLIS = false
// const strMAWDLISMirthChannel = 'OB_MAWDLIS_CDC_Plate_Result'
//Dev
const strMAWDLISMirthChannel = 'OB_MAWDLIS_CDC_Plate_Result'
const intValidRPValueCutoff = 40 // If RP < this value, considered valid
const intDebugLevel = 11 // 11 is all messages, 1 is critical only, all messages under this number will debug
const blDynamicallyDetectN2orRPFromFIle = false
// const strMODE = 'DEV'
...
// 7300
var strOutputPath = '/media/windowsshare/procedureinterface/7Flex/Result/Result_'

var strResultTextFile = ''
var strMAWDLISResult = ''
var blRPCheck = false // Do not set checks here, it is calculated if dynamic selected above.
var blN2Check = false
var strSQL = ''
...
if (blDynamicallyDetectN2orRPFromFIle) {
  ...
  // End Verification.
  // N2 not normally used.
} else {
  // Assume RP is there and N2 is not.
  blRPCheck = true
  blN2Check = false
}

// N2 no longer checked, if it is logic here may need updated?
if (blN2Check && blRPCheck) {
  ...
  // This is where it will be loading 7900.
  ...
  for (var intRPLookupCounter = intLineStart; intRPLookupCounter < getArrayOrXmlLength(msg['row']); intRPLookupCounter++) {
    ...
    if (strTargetName === 'RP') {
      ...
      var strN1TargetName = ''
      for (var intN1LookupCounter = 0; intN1LookupCounter < getArrayOrXmlLength(msg['row']); intN1LookupCounter++) {
        // Loop 2 to get N1 values populated.
        strN1SampleName = msg['row'][intN1LookupCounter]['SampleName'].toString()
        PrintToDebugLog(7, 'N1 Sample Name:' + strN1SampleName + ' RP Sampe Name:' + strRPSampleName)
        if (strRPSampleName === strN1SampleName) {
          PrintToDebugLog(7, 'N1 RP Sample Name Match')
          ...
          if (strN1TargetName === 'N1' || strN1TargetName === 'COVID') {
            ...
            strN1CTValue = msg['row'][intN1LookupCounter]['CT'].toString()
            PrintToDebugLog(7, 'N1 RP Match')
            // Exit Loop after N1 found found
            break
          }
        } // End If RP Sample Names match.  If not match do nothing
      }
      // Keep logic here, just do not loop
      // N1 CT Value, Sample Name and RP CT Value already assigned.  Further looping is unnecessary, but logic for result status needs moved outside loop.
      if (strRPCTValue == parseFloat(strRPCTValue)) {
        // RP is Number - confirm less than cutoff value
        ...
        if (parseFloat(strRPCTValue) < parseFloat(intValidRPValueCutoff)) {
          ...
          // Loop below commented out.  No longer necessary.
          // for (var intN1LookupCounter = 0; intN1LookupCounter < getArrayOrXmlLength(msg['row']); intN1LookupCounter++) {
          // Loop 3
          ...
          if (strRPSampleName === strN1SampleName) {
            if (strN1TargetName === 'N1' || strN1TargetName === 'COVID') {
              ...
              // Logic for detected not detected
              if (strN1CTValue === 'Undetermined') {
              ...
              // End Logic for detected vs not detected.
            }
          } else {
            PrintToDebugLog(7, 'N1 name did not match')
          }
          // Loop below unnecessary, commented out.
          // }
        } else {
          // RP CT is not less than 32
          // HOLD
...
function SendToInterfaceAndBuildTextFile (strLocalSampleName, strLocalProcResult, strLocalN1CTValue, strLocalN2CTValue, strLocalRPCTValue, strLocalComment, strLocalRPWell, strLocalN1Well, strLocalN2Well) {
  ...
  if (strLocalSampleName !== '') {
    ...
    if (blSaveResultTextFile) {
      strResultTextFile = strResultTextFile + strLocalSampleName + ': ' + strLocalProcResult + '-' + strLocalComment + ' Msg ID:' + channelName + '_' + connectorMessage.getMessageId() + ' \r\n'
    }
    ...
    if (blDBUpload) {
      try {
        ...
        strSQL = "INSERT INTO UrgentProcedureTracking.tblCDCResults \
          ...
          " + strRPWellForDB + ",\
          " + strCommentForDB + ",\
          '" + channelName + "_" + connectorMessage.getMessageId() + "',\
          " + blSentToLIS + ");"
